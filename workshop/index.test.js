test("makeUrl creates pokeAPI url correctly", t => {
  t.equal(makeUrl("test"), "https://pokeapi.co/api/v2/test");
});
test("searchParamsToObj creates objects correctly", t => {
  ...
  t.equal(
    result.name,
    expected.name,
    `Expected name value "${expected.name}" matches actual name value "${result.name}"`
  );
  t.equal(
    result.email,
    expected.email,
    `Expected email value "${result.email}" matches actual name value "${expected.email}"`
  );
});
test("isLeapYear should handle invalid input", t => {
  t.equal(isLeapYear("hi"), "Year must be an number", "Input must be a number");
  t.equal(
    isLeapYear(-1),
